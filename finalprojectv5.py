...
def main():
    ...
    entry.pack()
    selected_option = tk.StringVar(value="Add")     #creates a base variable for the radiobutton
    tk.Radiobutton(window, text="Add", variable=selected_option, value="Add").pack()  #radiobuttons for the two options
    tk.Radiobutton(window, text="Study", variable=selected_option, value="Study").pack()
    button = tk.Button(window, text="Submit", command=lambda: get_value(selected_option, entry)) #calls the getvalue function 
    button.pack()
    window.mainloop()
def add():  #Add function for adding flashcards
    ...
    flashcard_name_label = tk.Label(add_window, text="Enter the Flashcard Set Name:")
    flashcard_name_label.pack()
    flashcard_name_entry = tk.Entry(add_window)
    flashcard_name_entry.pack()
    folder_button = tk.Button(add_window, text="Browse", command=select_folder)
    folder_button.pack()
...
def study():
    flashcard_set = input("Which flashcard set would you like to study?") + ".txt" #currently typed into the console need to add into tk
    image_paths = load_image_paths(flashcard_set) #calls function
    study_window = tk.Toplevel() #creates new flashcard study window
    study_window.title("Flashcard Study")
